...
function scssTask() {
  return src(files.scssPath)
    ...
    .pipe(
      dest("src/ui/tmp") //put in temporary directory
    );
}

// JS task: concatenates JS files to scripts.js (minifies on production build)
function jsTask() {
  return src(files.jsPath).pipe(concat("scripts.js")).pipe(dest("src/ui/tmp"));
}
...
function manifestTask() {
  return src([files.manifest]).pipe(dest("dist"));
}

// Watch all key files for changes, if there is a change saved, create a build
function watchTask() {
  watch(
    [files.scssPath, files.jsPath, files.tsPath, files.html, files.manifest],
    { interval: 1000, usePolling: true },
    series(parallel(jsTask, tsTask), scssTask, htmlTask, manifestTask, cleanUp)
  );
}
